...
import { CancelInvitationUseCase } from '@/modules/user-management/application/use-cases/invitation/cancel-invitation.usecase.js'
...
describe('CancelInvitationUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new CancelInvitationUseCase(invitationService)
  })

  describe('execute', () => {
    it('should throw if invitationId is missing', (done) => {
      useCase.execute('', 'user-123').subscribe({
        next: () => done.fail('Should have thrown'),
        error: (err) => {
          expect(err.message).toBe('Invitation ID and user ID are required')
          done()
        },
      })
    })
    ...
    it('should return result on success', (done) => {
      ...
      invitationService.cancelInvitation.mockReturnValue(of(invitation))

      useCase.execute('inv-123', 'user-123').subscribe({
        next: (result) => {
